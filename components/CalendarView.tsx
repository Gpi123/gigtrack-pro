...
const CalendarView: React.FC<CalendarViewProps> = ({ gigs, selectedDate, onDateSelect, showValues }) => {
    ...
    days.push(
      <button
        ...
        className={`relative h-10 w-10 sm:h-12 sm:w-12 flex flex-col items-center justify-center rounded-xl transition-all hover:bg-slate-800 ${
          isSelected ? 'bg-indigo-600 text-white ring-2 ring-indigo-400 ring-offset-2 ring-offset-slate-900 shadow-lg' : 
          isToday ? 'bg-slate-800 text-indigo-400 font-bold border border-indigo-500/30' : 'text-slate-300'
        }`}
      >
      ...
      {selectedDate && (
        <div className="mt-6 pt-6 border-t border-slate-800 animate-in slide-in-from-top-2">
          <div className="flex justify-between items-center mb-4">
            <span className="text-xs text-slate-400 font-medium">Shows em <span className="text-indigo-400 font-bold">{new Date(selectedDate).toLocaleDateString('pt-BR')}</span></span>
            <button 
              ...
              selectedDayGigs.map(gig => (
                <div key={gig.id} className="bg-slate-950/50 border border-slate-800 rounded-2xl p-3 flex flex-col gap-2">
                  <div className="flex justify-between items-start gap-2">
                    ...
                    <span className="text-xs font-black text-indigo-400 whitespace-nowrap">{formatCurrency(gig.value)}</span>
                  </div>
